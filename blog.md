...
- 优化了同步网络节点列表的开销。前面的版本，在网络io上应该是随着节点数增长呈现指数级的增长，这个问题，今天的版本应该会好转很多。  
- 优化了因为节点同时发起请求，导致数据不能正确同步的bug（主要是请求状态同步的bug）。  

### 2019-05-27

最近开始处理一些相对底层的模块，最先是日志系统（配置系统可能也算，这些其实都算app系统的一部分），接下来应该是app系统（以前的script应该会逐步的替换掉），再然后就是file系统（分布式的文件系统，但没必要做特别复杂，毕竟文件存储不是这个网络最需要保障的，只是为上层任务提供一个方便而已，后面如果上层业务逻辑有明确需求，可以进一步对接更好用的分布式文件系统），这几块处理完，还会需要回头处理网络层的整合，也就是彻底的点对点，每个节点的数据需要统一起来，不是简单的request-reply（现在其实已经有类似问题导致的并发bug了，毕竟现在是对等的P2P网络，很难像以前CS或BS结构那样处理并发，P2P要更复杂一些），而是彻底的点对点的数据流，然后把代理转发处理对，所有这些都完成，这个内核层才算基本完成。

### 2019-05-25

今天发现在sendmsg阶段，如果某个节点出问题，其实没有进入connectfail流程，这样会导致大量的连接重试。  

然后，因为requestnodestate有并发问题，主要是2个节点可能同时主动发起请求，虽然其实对逻辑没影响，但还是调整了一下。
